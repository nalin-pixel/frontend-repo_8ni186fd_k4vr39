...
function App() {
  return (
    <div className="min-h-screen w-full overflow-hidden bg-[#2f9e3d] flex items-center justify-center">
      {/* Inline styles for SVG animations */}
      <style>{`
        @keyframes flameFlicker { 0% { transform: scale(1) translateY(0); opacity: 0.95; } 50% { transform: scale(1.06) translateY(-2px); opacity: 1; } 100% { transform: scale(1) translateY(0); opacity: 0.95; } }
        @keyframes smokeDrift { 0% { transform: translate(0,0) scale(1); opacity: .28; } 50% { transform: translate(6px,-10px) scale(1.08); opacity: .18; } 100% { transform: translate(0,0) scale(1); opacity: .28; } }
        @keyframes wingPulse { 0% { transform: rotate(0deg) translateY(0); } 50% { transform: rotate(-1.2deg) translateY(1px); } 100% { transform: rotate(0deg) translateY(0); } }
        @keyframes tailSway { 0% { transform: rotate(0deg); } 50% { transform: rotate(1.5deg); } 100% { transform: rotate(0deg); } }
      ...
      {/* Full-screen plain green grass field */}
      <div className="absolute inset-0">
        <svg className="w-full h-full" preserveAspectRatio="none" viewBox="0 0 1600 900">
          <defs>
            <linearGradient id="grassGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#46b14f" />
              <stop offset="60%" stopColor="#2f9e3d" />
              <stop offset="100%" stopColor="#238a31" />
            </linearGradient>
          ...
          <rect x="0" y="0" width="1600" height="900" fill="url(#grassGrad)" filter="url(#grassNoise)"/>
          {/* Foreground blades for realism while keeping the area plain */}
          <g opacity="0.85">
            {Array.from({ length: 140 }).map((_, i) => {
              const x = 8 + i * 12
              const h = 28 + ((i * 37) % 60)
              const offset = ((i * 97) % 16) - 8
              return (
                <path key={i} d={`M ${x} 900 q ${offset} -${h/2} 0 -${h} q ${-offset} ${h/2} 0 ${h}`} stroke="#1d7a2a" strokeWidth="2" fill="none" opacity="0.35"/>
              )
            })}
          </g>
        </svg>
      ...
      {/* Dragon + Fire */}
      <div className="relative w-full max-w-[1200px] h-[78vh]">
